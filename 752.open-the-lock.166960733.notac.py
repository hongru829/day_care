...
class Solution(object):
    def openLock(self, deadends, target):
        ...
        while queue:
            size = len(queue)
            for k in range(size):
                nextPassCode = queue.pop(0)
                if nextPassCode == target:
                    return step
                
                for i in range(len(nextPassCode)):
                    for j in range(-1, 2, 2):
                        updatedNumber = (int(nextPassCode[i]) + j + 10) % 10
                        updatedNextPassCode = nextPassCode[:i] + str(updatedNumber) + nextPassCode[i+1:]
                        
                        if updatedNextPassCode not in visited and updatedNextPassCode not in deadends:
                            visited.add(updatedNextPassCode)
                            queue.append(updatedNextPassCode)
            step += 1
        return -1
