...
class Solution(object):
    def getImportance(self, employees, id):
        ...
        :rtype: int
        """
        employees = {employee.id: (employee.importance, employee.subordinates) for employee in employees}
        queue = [employees[id]]
        res = 0
        visited = set()
        ...
        while queue:
            size = len(queue)
            for k in range(size):
                nextEmployee = queue.pop(0)
                res += nextEmployee[0]
                if len(nextEmployee[1]) > 0:
                    for id in nextEmployee[1]:
                        queue.append(employees[id])
        return res
