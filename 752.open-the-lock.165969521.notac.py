...
class Solution(object):
    def openLock(self, deadends, target):
        ...
        while queue:
            ...
            count += 1
            for k in range(size):
                nextWheel = queue.pop(0)
                for i in range(4):
                    for j in range(-1, 2, 2):
                        updatedNumber = (int(nextWheel[i]) + j + 10) % 10
                        updatedWheel = nextWheel[:i] + str(updatedNumber) + nextWheel[i+1:]
                        if updatedWheel == target:
                            return count
                        if updatedWheel in deadends or updatedWheel in visited:
                            continue
                        queue.append(updatedWheel)
                        visited.add(updatedWheel)
        return -1
